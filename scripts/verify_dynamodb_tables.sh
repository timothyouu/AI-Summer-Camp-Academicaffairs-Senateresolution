...
verify_table "policy-intelligence-conflicts" \
  '{"conflict_id":{"S":"healthcheck_conflict"}}' \
...
verify_table "policy-intelligence-access-control" \
  '{"user_id":{"S":"healthcheck_user"},"source_key":{"S":"healthcheck_source"}}' \
  'attribute_not_exists(#pk) AND attribute_not_exists(#sk)' \
  '{"#pk":"user_id","#sk":"source_key"}'
verify_table "policy-intelligence-source-registry" \
  '{"source_id":{"S":"healthcheck_source_registry"}}' \
  'attribute_not_exists(#pk)' \
  '{"#pk":"source_id"}'
verify_table "policy-intelligence-draft-versions" \
  '{"draft_id":{"S":"healthcheck_draft"},"version_id":{"S":"healthcheck_version"}}' \
  'attribute_not_exists(#pk) AND attribute_not_exists(#sk)' \
  '{"#pk":"draft_id","#sk":"version_id"}'

verify_gsi "policy-intelligence-access-control" "source-key-user-index"
verify_gsi "policy-intelligence-source-registry" "status-last-synced-index"
verify_gsi "policy-intelligence-source-registry" "source-type-last-synced-index"
verify_gsi "policy-intelligence-source-registry" "owner-last-synced-index"
verify_gsi "policy-intelligence-draft-versions" "owner-updated-index"
