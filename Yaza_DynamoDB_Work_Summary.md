...
> **Integration note (2026-07-15, merged into `prod`).** This document is Yaza's
> record of the app-memory branch as it was built, and is kept as written. What
> actually landed differs in three ways — read this note first, then treat the
> sections below as the design rationale rather than the current schema.
>
> - **Feedback and recurring questions landed as designed.** Both stores, both
>   tables (`feedback_id` / `question_id` partition keys), `dynamodb_client.py`,
>   the chat `answer_id`, and the wired-up frontend thumbs are all in `prod`.
>   These are the Notion Should-Have items "Answer feedback" and "Recurring
>   questions hub"; the thumbs were dead UI before this work.
> - **§10/§11 are out of date.** They list the source-registry, access-control,
>   and draft-version APIs as "not implemented yet" and suggest them as next
>   commits. They already existed on `prod` (`registry.py`, `permissions.py`,
>   `drafting.py`, shipped as PRD Round-2 / implementation3.md Task 11) against
>   different key schemas: `id` rather than `source_id`, `user_email`+`source_type`
>   rather than `user_id`+`source_key`, and `draft_id`+numeric `version` rather
>   than `draft_id`+`version_id`. Since those APIs and their tests already worked
>   and the tables held no data, the existing schemas won and the four
>   overlapping tables in §3 were re-provisioned. `policy-intelligence-uploads`
>   was added; it was missing from this set.
> - **The APP_ENV / APP_PERSISTENCE_BACKEND switch in §4 and §7 is gone.** The
>   repo gates each table independently on its own `DDB_*_TABLE` variable, so a
>   single feature can move to AWS on its own. The `DYNAMODB_*` names in §7 are
>   still accepted as aliases and still work — see `backend/.env.example`.
>
> `scripts/setup_dynamodb_tables.sh` and `verify_dynamodb_tables.sh` survive and
> now carry the merged schemas. Setup refuses to touch a table whose key schema
> predates the merge instead of silently leaving one the backend cannot read.
