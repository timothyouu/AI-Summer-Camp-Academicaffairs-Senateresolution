...
> Integration note: this work originally branched from `demo`. The
> `integration/yaza-dynamodb-app-memory` branch reconciles it with the newer
> Bedrock/Strands/Cognito/CDK architecture. Current `DDB_CONFLICTS_TABLE` and
> `DDB_UPLOADS_TABLE` behavior is preserved; feedback and recurring questions
> use the added application-memory stores, while the remaining table schemas are
> retained for upcoming source lifecycle, permissions, and draft-version APIs.
