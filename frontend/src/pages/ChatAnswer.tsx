...
export default function ChatAnswer() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setLoadError("");
      setFollowUp("");
      if (followUpInputRef.current) followUpInputRef.current.style.height = "auto";
    } catch (reason) {
    ...
    <section className="mx-auto max-w-[806px] pb-3 text-navy">
      ...
      <form onSubmit={submit} className="mt-10 rounded-2xl border border-navy/20 bg-white px-5 py-4 shadow-card">
        ...
        <textarea
          ...
          className="block max-h-60 min-h-6 w-full resize-none overflow-y-auto bg-transparent text-base leading-6 text-navy outline-none placeholder:text-navy/55"
        />
