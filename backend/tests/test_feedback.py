...
def feedback_settings() -> PersistenceSettings:
...
def test_feedback_store_factory_selects_sqlite_without_aws(
    ...
    assert isinstance(get_feedback_store(load_persistence_settings({})), SQLiteFeedbackStore)


def test_feedback_store_factory_selects_dynamodb_lazily() -> None:
    assert isinstance(get_feedback_store(feedback_settings()), DynamoDBFeedbackStore)
...
def test_dynamodb_feedback_store_creates_and_lists_with_stubbed_table() -> None:
    ...
    store = DynamoDBFeedbackStore(feedback_settings(), resource=resource)
