...
def recurring_settings() -> PersistenceSettings:
    return load_persistence_settings(
        {
            "APP_PERSISTENCE_BACKEND": "dynamodb",
            "DYNAMODB_RECURRING_QUESTIONS_TABLE": "recurring-questions-test",
        }
    )
...
def test_recurring_question_store_factory_selects_sqlite_without_aws(
    ...
    assert isinstance(get_recurring_question_store(load_persistence_settings({})), SQLiteRecurringQuestionStore)
...
def test_dynamodb_recurring_question_store_records_and_aggregates_with_stubbed_table() -> None:
    ...
    store = DynamoDBRecurringQuestionStore(recurring_settings(), resource=resource)
...
def test_chat_survives_recurring_question_storage_failure(
    ...
    monkeypatch.setattr("backend.app.chat.get_recurring_question_store", lambda: BrokenStore())
